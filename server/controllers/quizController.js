...
export const generateQuiz = async (req, res) => {
  ...
  const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
    ...
    const prompt = `
Based ONLY on the following document content, generate exactly 100 multiple-choice quiz questions.
...
- Make sure questions are varied in difficulty and relevant to the content.

Document Content:
${extractedText.substring(0, 8000)}

Respond strictly as a JSON array in this format:
[
  {
...
`;
    ...
    try {
     ...
     try {
    questions = parse(aiResponse);
  } catch (innerErr) {
    console.error("❌ Failed to parse AI response:", aiResponse.slice(0, 500));
    throw new Error("Invalid JSON format returned by AI. Please retry.");
  }
    }
    ...
    // 🧩 Format questions with fallback values
    const formattedQuestions = questions.slice(0, 100).map((q, i) => ({
      id: `q-${Date.now()}-${i}`,
